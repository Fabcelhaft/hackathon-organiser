...
- **FR-005**: The project MUST include a `.devcontainer/devcontainer.json` that gives developers a fully configured local environment (Java, Maven, Docker CLI) without manual installation steps. PostgreSQL is available on demand by running `docker compose up db` from inside the container using the Docker CLI provided by the devcontainer. Port `8080` MUST be listed in `forwardPorts` so the running application is reachable from the host browser.
