...
- [X] T001 Append the idempotent column addition to `src/main/resources/schema.sql` (after the feature 006 block, with a `-- Feature 009: Sortable Custom Fields (data-model.md; FR-001, FR-002)` comment): `ALTER TABLE custom_field_definitions ADD COLUMN IF NOT EXISTS sort_index integer NOT NULL DEFAULT 0;` — research.md §1
- [X] T002 Add `private int sortIndex;` with `getSortIndex()`/`setSortIndex(int)` to `src/main/java/net/fabcelhaft/hackathonorganiser/customfield/CustomFieldDefinition.java` (no `@Column` needed — the default naming strategy maps it to `sort_index`, as `ContentPage.sortIndex` already does); extend the class Javadoc with one sentence on the attribute's role — data-model.md "Custom Field Definition"
...
- [X] T017 [P] [US2] Also in `ProfileManagementIT` add `mixedCaseLabelsAtTheSameIndexSortCaseInsensitively`: fixtures "Banana"@0 then "apple"@0 → on `/profile/edit`, `apple` precedes `Banana` (spec US2 scenario 4)
...
- [X] T020 [P] [US3] In `src/test/java/net/fabcelhaft/hackathonorganiser/organiser/participant/ParticipantManagementIT.java` add `organiserDetailListsCustomFieldsInSortIndexThenLabelOrder`: fixtures Omega@3, Mid@0, Zeta@-1, Alpha@0 (creation order as written), one participant; GET `/organiser/participants/{id}` as organiser → label `indexOf` positions Zeta < Alpha < Mid < Omega
- [X] T021 [P] [US3] In `src/test/java/net/fabcelhaft/hackathonorganiser/organiser/compliance/ComplianceManagementIT.java` add `fieldDropdownListsCustomFieldsInSortIndexThenLabelOrder`: same fixtures via `persistDefinition(String)` + `setSortIndex` + re-save; GET `/organiser/compliance` → within the `<select>` the `<option>` texts appear Zeta < Alpha < Mid < Omega

### Implementation for User Story 3

- [X] T022 [US3] Confirm by reading that `src/main/java/net/fabcelhaft/hackathonorganiser/organiser/compliance/ComplianceController.java` (`renderForm`, `customFieldService.findAll()` → stream filter) and `src/main/java/net/fabcelhaft/hackathonorganiser/organiser/participant/ParticipantController.java` (`participantService.findDetail`) consume ordered sources and need no edit; add a one-line Javadoc note on `ComplianceController.renderForm` that `availableFields` preserves `CustomFieldDefinition.DISPLAY_ORDER` (FR-009)
- [X] T023 [US3] Run `ParticipantManagementIT` and `ComplianceManagementIT`; confirm T020/T021 pass (they should pass as soon as T018 is in place — if they pass before T018, the fixture is not exercising the order: fix the fixture, not the assertion)

**Checkpoint**: All views agree (SC-002).

---

## Phase 5b: User Story 4 - Organiser orders the options of a select field (Priority: P2)

> Added during implementation after the user clarified that select options need ordering too (spec
> Clarifications, third entry; research.md §7). Task IDs continue after the Polish phase's because Polish was
> numbered first; execute this phase **before** Phase 6.

**Goal**: Each option of a single/multi-select field has its own sort index, set when adding the option or
afterwards on the field's edit page, and options are offered/shown in index-then-label-then-creation order
everywhere.

**Independent Test**: Create a single-select field with options L, M, S; set S=1, M=2, L=3 on the edit page;
open `/register` and confirm the radio choices read S, M, L (quickstart.md §10).

### Tests for User Story 4 (write first, confirm they FAIL)

- [X] T028 [P] [US4] In `src/test/java/net/fabcelhaft/hackathonorganiser/customfield/CustomFieldServiceTest.java`: update the existing `addOption(definitionId, "java")` call to the new arity (`, 0`); add (a) `findOptionsOrdersBySortIndexThenLabelThenCreation` — repository returns L@3, S@1, M@2 plus "apple"@1 and "Banana"@1 → S, apple, Banana, M, L; (b) `addOptionPersistsTheGivenSortIndex`; (c) `updateOptionSortIndexChangesOnlyTheIndex` — option belongs to the definition, index becomes 7, label unchanged; (d) `updateOptionSortIndexCompletesEmptyForAnOptionOfAnotherDefinition` — no save; (e) `createStoresInitialOptionsAtIndexZero`
- [X] T029 [P] [US4] In `src/test/java/net/fabcelhaft/hackathonorganiser/organiser/customfield/CustomFieldManagementIT.java` add: (a) POST `/{id}/options` with `label=M&sort_index=2` → 303 and stored index 2; (b) POST without `sort_index` → stored 0; (c) POST with `sort_index=abc` → 200, body contains the error, option count unchanged; (d) POST `/{id}/options/{optionId}` with `sort_index=7` → 303 and stored 7, label unchanged; (e) same with `sort_index=x` → 200 + error, index unchanged; (f) POST `/{id}/options/{optionId}` where the option belongs to another definition → 404; (g) GET `/{id}/edit` with options L@3, S@1, M@2 → body positions S < M < L and each row shows its index; (h) `nonOrganiserIsDeniedOnEveryRoute` gains the new route
- [X] T030 [P] [US4] In `src/test/java/net/fabcelhaft/hackathonorganiser/participants/RegistrationManagementIT.java` add `selectOptionsAreOfferedInSortIndexOrder`: single-select with options persisted L@3, M@2, S@1 (add a `persistOption(definitionId, label, sortIndex)` overload) → GET `/register` body positions S < M < L
- [X] T031 [P] [US4] In `src/test/java/net/fabcelhaft/hackathonorganiser/participants/ParticipantsDirectoryManagementIT.java` add `selectedOptionsAreShownInSortIndexOrder`: multi-select overview+public field with options L@3, S@1, M@2, a participant who selected L and S → GET `/participants` cell shows S before L; GET `/participants/{id}` likewise
- [X] T032 [P] [US4] In `src/test/java/net/fabcelhaft/hackathonorganiser/participant/ParticipantServiceTest.java` add a lenient `customFieldService.findOptions(any(UUID.class))` → `Flux.empty()` stub to `stubNoStoredFieldValues()` (rendering loads now go through the service; the validation-only repository stub stays)

### Implementation for User Story 4

- [X] T033 [US4] Append `ALTER TABLE custom_field_options ADD COLUMN IF NOT EXISTS sort_index integer NOT NULL DEFAULT 0;` to `src/main/resources/schema.sql` under the feature 009 comment; add `int sortIndex` + accessors + `DISPLAY_ORDER` comparator (index, label case-insensitive, createdAt nullsLast) to `src/main/java/net/fabcelhaft/hackathonorganiser/customfield/CustomFieldOption.java`
- [X] T034 [US4] In `src/main/java/net/fabcelhaft/hackathonorganiser/customfield/CustomFieldService.java`: `findOptions()` sorts by `CustomFieldOption.DISPLAY_ORDER`; `addOption(id, label, int sortIndex)`; `saveOption(definitionId, label, sortIndex)` (initial options from `create` pass 0); new `updateOptionSortIndex(definitionId, optionId, sortIndex)` that loads the option, checks `getCustomFieldDefinitionId().equals(definitionId)` (else empty), sets the index + `updatedAt`, saves
- [X] T035 [US4] In `src/main/java/net/fabcelhaft/hackathonorganiser/organiser/customfield/CustomFieldController.java`: `addOption` parses `sort_index` at the head of its chain (same shape as create/update) and passes it on; new `@PostMapping("/{id}/options/{optionId}") updateOptionSortIndex(...)` with the same parse, `switchIfEmpty` → 404, success 303 to the edit page, `CustomFieldConflictException` → `renderEditFormAfterOptionError`
- [X] T036 [US4] In `src/main/resources/templates/organiser/custom-fields/form.html`: each `existingOptions` row shows the label, its index, an inline `<form method="post" th:action="@{/organiser/custom-fields/{id}/options/{optionId}(...)}">` with `<input type="number" name="sort_index" step="1" th:value="${option.sortIndex}">` + Save, then the existing Remove form; the add-option form gains `<input type="number" name="sort_index" step="1" value="0">`
- [X] T037 [US4] In `src/main/java/net/fabcelhaft/hackathonorganiser/participant/ParticipantService.java` switch `loadFieldViews` and `blankFieldViews` from `customFieldOptionRepository.findByCustomFieldDefinitionId(...)` to `customFieldService.findOptions(...)`; leave the two validation loads as they are
- [X] T038 [US4] Run `CustomFieldServiceTest`, `ParticipantServiceTest`, `CustomFieldManagementIT`, `RegistrationManagementIT`, `ParticipantsDirectoryManagementIT`; confirm T028–T032 pass and nothing regressed

**Checkpoint**: Options are ordered everywhere (SC-006).

---

## Phase 6: Polish & Cross-Cutting Concerns

- [X] T024 Run the full suite `mvn -B verify` from the repository root and confirm green; commit
...
- [X] T026 [P] Re-read `specs/009-custom-fields-sortable/spec.md` Functional Requirements FR-001–FR-014 against the diff and tick each off in a short checklist comment in the PR description; confirm FR-014 (no audit) holds — `git grep -n "auditService" src/main/java/net/fabcelhaft/hackathonorganiser/customfield` must return nothing
- [X] T027 [P] Verify a fresh database (drop the Testcontainers/dev volume, start the app) and an existing database (pre-feature rows) both come up with every definition at `sort_index = 0` and purely alphabetical views (SC-003, FR-002)
